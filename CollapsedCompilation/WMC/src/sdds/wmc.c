...
SddWmc wmc_propagate(WmcManager* wmc_manager) {
  ...
  iters += 1;
  printf("iters: %lld, operations: %lld, ops per propagation: %lf\n", iters, count, (1.0*count) / iters);
...
void update_derivatives_of_missing(SddWmc drv_wmc, Vtree* vtree, Vtree* sub_vtree, WmcManager* wmc_manager) {
  ...
  if(vtree==sub_vtree || vtree->no_var_in_sdd) return;
    ...
    INC_LIT_DRV(var,drv_wmc,wmc_manager); count += 1;
    INC_LIT_DRV(-var,drv_wmc,wmc_manager); count += 1;
  }
  else {
    SddWmc l_wmc = MULT(drv_wmc,USED_TRUE_WMC(vtree->left,wmc_manager)); count += 1;
    SddWmc r_wmc = MULT(drv_wmc,USED_TRUE_WMC(vtree->right,wmc_manager)); count += 1;
    
    if(sub_vtree!=NULL && sdd_vtree_is_sub(sub_vtree,vtree)) {
      SddWmc s_wmc = USED_TRUE_WMC(sub_vtree,wmc_manager);
      if(sdd_vtree_is_sub(sub_vtree,vtree->left)) { l_wmc = DIV(l_wmc,s_wmc); count += 1; }
      else { r_wmc = DIV(r_wmc,s_wmc); count += 1; }
    }
