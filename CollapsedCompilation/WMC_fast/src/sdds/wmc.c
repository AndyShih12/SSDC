...
static void finish_derivatives_of_missing(WmcManager* wmc_manager);
...
//can only be used when A >= B
#define EPS 1e-8
#define SUB(A,B) (log_mode? (IS_ZEROW(B)? A: (A-B < EPS ? ZEROW : A+log1p(-1*exp(B-A)))): (A-B))
#define DEC(A,B) A = SUB(A,B)
...
WmcManager* wmc_manager_new(SddNode* node, int lm, SddManager* manager) {
  ...
  //allocate memory for var to index bookkeeping
  CALLOC(wmc_manager->var_to_index,SddLiteral,manager->var_count+1,"wmc_manager_new");
  CALLOC(wmc_manager->index_to_var,SddLiteral,manager->var_count+1,"wmc_manager_new");

  //allocate memory for range update cache
  CALLOC(wmc_manager->drv_cache_pos,SddWmc,manager->var_count+2,"wmc_manager_new");
  CALLOC(wmc_manager->drv_cache_neg,SddWmc,manager->var_count+2,"wmc_manager_new");
...
void wmc_manager_free(WmcManager* wmc_manager) {
  ...
  free(wmc_manager->var_to_index);
  free(wmc_manager->index_to_var);
  free(wmc_manager->drv_cache_pos);
  free(wmc_manager->drv_cache_neg);
...
void initialize_wmc(WmcManager* wmc_manager) {
  ...
  //calculate index to variable mappings (left-to-right order of vtree leaves)
  int ind = 1;
  Vtree* v_ptr = wmc_manager->sdd_manager->vtree->first;
  while (v_ptr) {
    if (LEAF(v_ptr)) {
      wmc_manager->index_to_var[ind] = v_ptr->var;
      wmc_manager->var_to_index[v_ptr->var] = ind;
      ind += 1;
    }
    v_ptr = v_ptr->next;
  }
  assert(ind == VAR_COUNT(wmc_manager) + 1);

  // store pos and neg separately since log_mode does not support negative numbers
  for(int i = 0; i < VAR_COUNT(wmc_manager)+1; i++) {
    wmc_manager->drv_cache_pos[i] = ZEROW;
    wmc_manager->drv_cache_neg[i] = ZEROW;
  }
}
...
SddWmc wmc_propagate(WmcManager* wmc_manager) {
  ...
  finish_derivatives_of_missing(wmc_manager);

  iters += 1;
  printf("iters: %lld, ops: %lld, ops per propagation: %lf\n", iters, count, (1.0*count)/iters);
...
void update_derivatives_of_missing(SddWmc drv_wmc, Vtree* vtree, Vtree* sub_vtree, WmcManager* wmc_manager) {
  ...
  // Get the product of missing wmc's.
  SddWmc product = wmc_of_missing(drv_wmc, vtree, sub_vtree, wmc_manager);

  int l_vtree, l_sub_vtree, r_sub_vtree, r_vtree;

  // get the two intervals [l1,r1), [l2,r2) to fill in
  // where l1 = l_vtree, r1 = l_sub_vtree, l2 = r_sub_vtree, r2 = r_vtree
  l_vtree = wmc_manager->var_to_index[vtree->first->var];
  r_vtree = wmc_manager->var_to_index[vtree->last->var] + 1;

  if (sub_vtree != NULL) {
    l_sub_vtree = wmc_manager->var_to_index[sub_vtree->first->var];
    r_sub_vtree = wmc_manager->var_to_index[sub_vtree->last->var] + 1;

    if (l_sub_vtree != l_vtree) {
      INC(wmc_manager->drv_cache_pos[l_vtree], product); count += 1;
      INC(wmc_manager->drv_cache_neg[l_sub_vtree], product); count += 1;
    }
    if (r_sub_vtree != r_vtree) {
      INC(wmc_manager->drv_cache_pos[r_sub_vtree], product); count += 1;
      INC(wmc_manager->drv_cache_neg[r_vtree], product); count += 1;
    }
  }
  else {
    INC(wmc_manager->drv_cache_pos[l_vtree], product); count += 1;
...
static
void finish_derivatives_of_missing(WmcManager* wmc_manager) {
  int n = VAR_COUNT(wmc_manager) + 1;

  // range update
  SddWmc drv_wmc = ZEROW;
  for (int i = 1; i < n; i++) {
    if (wmc_manager->drv_cache_pos[i] != ZEROW) { 
      INC(drv_wmc, wmc_manager->drv_cache_pos[i]); count += 1; 
      wmc_manager->drv_cache_pos[i] = ZEROW;
    }
    if (wmc_manager->drv_cache_neg[i] != ZEROW) { 
      DEC(drv_wmc, wmc_manager->drv_cache_neg[i]); count += 1; 
      wmc_manager->drv_cache_neg[i] = ZEROW;
    }

    SddLiteral var = wmc_manager->index_to_var[i];
    SddNode* lit = sdd_manager_literal(var, wmc_manager->sdd_manager);
    Vtree* vtree = lit->vtree;

    if(vtree->all_vars_in_sdd) { //used var
      SddWmc sum = USED_TRUE_WMC(vtree,wmc_manager);
      SddWmc drv_wmc_tmp = DIV(drv_wmc, sum); count += 1;
      INC_LIT_DRV(var, drv_wmc_tmp, wmc_manager); count += 1;
      INC_LIT_DRV(-var, drv_wmc_tmp, wmc_manager); count += 1;
      //printf("incr: %lf, var: %ld\n", drv_wmc_tmp, var);
    }
  }
  //printf("drv_wmc: %lf, left: %lf\n", drv_wmc, wmc_manager->drv_cache_neg[0]);
  //printf("range: %lld, count: %lld, vars: %d\n", range, count, n);
}
